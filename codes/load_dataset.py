...
import sys
from codes.Configures import data_args
...
def get_dataset(dataset_root, dataset_name):
    ...
    elif dataset_name.lower() in MoleculeNet.names.keys():
        return MoleculeNet(root=dataset_root, name=dataset_name)
    elif dataset_name.lower()  in ["proteins", "enzymes", "collab", "imdb-binary", "reddit-binary"]:
        return TUDataset(root=dataset_root, name=dataset_name)
    elif dataset_name.lower() in ['graph-sst2', 'graph_sst5', 'graph_twitter']:
        return SentiGraphDataset(root=dataset_root, name=dataset_name)
    elif dataset_name.lower() in ['ba_shapes', 'ba_community', 'ba_4motifs']:
...
def get_dataloader_bak(dataset, data_args, train_args):
